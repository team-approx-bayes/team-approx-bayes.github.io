...
Wait. Of course, this is not always defined! It is, say, if $\int \|\Phi(x)\|_{\mathcal{H}} P(\mathrm{d} x) <+\infty$.

Now, it appears that some kernels $k$ are known such that:

(1) $k(x,x) = \|\Phi(x) \|_{\mathcal{H}}^2 \leq 1 $ for any $x$, which in turn ensures that $\mu(P)$ is well defined for any $P$.
...
\begin{equation}
\hat{P}_{n} = \frac{1}{n}\sum_{i=1}^{n} \delta_{X_i}.
\end{equation}
...
\begin{equation}
\mathbb{E} \left[d_k^2\left(\hat{P}_n,P^0 \right)\right] = \mathbb{E} \left[ \| \frac{1}{n} \sum_{i=1}^n \Phi(X_i)-\mathbb{E}_{X\sim P^0}[\Phi(X)]  \|_{\mathcal{H}}^2 \right] = \mathrm{Var}\left(\frac{1}{n} \sum_{i=1}^n \Phi(X_i) \right)
\end{equation}

and so, as $X_1,\dots,X_n$ are i.i.d, 

\begin{equation}
\mathbb{E} \left[d_k^2\left(\hat{P}_n,P^0 \right)\right] = \frac{1}{n} \mathrm{Var}[\Phi(X_1)] \leq \frac{1}{n} \mathbb{E}\left[ \|\Phi(X_i) \|_\mathcal{H}^2\right] \leq \frac{1}{n}.
\end{equation}
