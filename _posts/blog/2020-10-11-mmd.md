...
We now prove that, as long as the kernel satisfies conditions (1) and (2) above, for any statistical model $(P_\theta,\theta\in\Theta)$ (parametric, or not!), given $X_1,\dots,X_n$ drawn i.i.d from some $P^0$,
...
Another example is the total variation distance, used in the aforementioned paper by Yatracos [7]. Note that the procedure used by Barraud, Birgé and Sart in [8] is much more involved.
