...
(1) $k(x,x) = \left\lVert\Phi(x) \right\rVert_{\mathcal{H}}^2 \leq 1 $ for any $x$, which in turn ensures that $\mu(P)$ is well defined for any $P$.
...
$$
\mathbb{E} \left[d_k^2\left(\hat{P}_n,P^0 \right)\right] = \frac{1}{n} \mathrm{Var}[\Phi(X_1)] \leq \frac{1}{n} \mathbb{E}\left[ \left\lVert\Phi(X_i) \right\rVert_\mathcal{H}^2\right] \leq \frac{1}{n}.
$$
