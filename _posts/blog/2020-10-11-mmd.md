...
\begin{equation}
\label{equa:mu}
 \mu(P) = \int \Phi(x) P({\rm d} x) = \mathbb{E}_{X \sim P } [\Phi(X)].
\end{equation}
Wait. Of course, this is not always defined! It is, say, if $\int \|\Phi(x)\| P({\rm d} x) <+\infty$.

Now, it appears that some kernels $k$ are known such that:
\begin{itemize}
 \item $k(x,x) = \left\|\Phi(x) \right\|_{\mathcal{H}}^2 \leq 1 $ for any $x$, which in turn ensures that $\mu(P)$ is well defined for any $P$.
 \item $P\mapsto \mu(P)$ is one to one.
\end{itemize}
An example, when $\mathcal{X}=\mathbb{R}^d$, is the Gaussian kernel
$$
k(x,x') = \exp\left(- \frac{\|x-x'\|^2}{\gamma^2} \right)
$$
for some $\gamma>0$.

For any such a kernel,
$$
d_{k}(P,Q) = \|\mu(P) - \mu(Q) \|_{\mathcal{H}}
$$
...
Note 2: if you believed ``Close your eyes, push the button, it works'' above, you will of course be disappointed. Life is not that simple. The choice of the kernel $k$ is far from easy, and is of course context dependent.
...
\begin{align*}
d_k\left(P_{\hat{\theta}_n},P^0 \right)
& \leq d_k\left(P_{\hat{\theta}_n},\hat{P}_n \right) +  d_k\left(\hat{P}_n,P^0 \right)
\\
& \leq d_k\left(P_{\theta},\hat{P}_n \right) +  d_k\left(\hat{P}_n,P^0 \right)
\\
& \leq d_k \left(P_{\theta},P^0 \right) + 2 d_k\left(\hat{P}_n,P^0 \right).
\end{align*}
We used the triangle inequality at the first and third line, and the definition of $\hat{\theta}$ at the second line. Take the expectation of both sides, this gives:
$$
...
\begin{align*}
\left\{ \mathbb{E} \left[ d_k\left(\hat{P}_n,P^0 \right)\right]\right\}^2
& \leq \mathbb{E} \left[d_k^2\left(\hat{P}_n,P^0 \right)\right]
\\
& = \mathbb{E} \left[ \left\| \frac{1}{n} \sum_{i=1}^n \Phi(X_i)-\mathbb{E}_{X\sim P^0}[\Phi(X)]  \right\|_{\mathcal{H}}^2 \right]
\\
& = {\rm Var}\left(\frac{1}{n} \sum_{i=1}^n \Phi(X_i) \right)
\\
& = \frac{1}{n} {\rm Var}[\Phi(X_1)]
\\
& \leq \frac{1}{n} \mathbb{E}\left[ \|\Phi(X_i) \|_\mathcal{H}^2\right]
\\
& \leq \frac{1}{n}.
\end{align*}
...
Another example is the total variation distance, used in the aforementioned paper by Yatracos [7]. Note that the procedure used by baraud, Birg\'e and Sart in [8] is much more involved.
...
Our preprint on MMD-estimation and robustness study specifically the robustness properties of $\hat{\theta}$. Namely, in which cases is the dependence with respect to $n$ and $\varepsilon$ optimal in \eqref{equa:robust}? Moreover, in this paper, we also extend the consistency proof to the case where the observations are not independent.
...
$$
This leads to robust Bayesian estimators.
...
[7] ([Y. G. Yatracos (1985). Rates of convergence of minimum distance estimators and Kolmogorov's entropy. The Annals of Statistics, 768-774.](https://projecteuclid.org/download/pdf_1/euclid.aos/1176349553))

[8] ([Y. Baraud, L. Birg\'e and M. Sart (2017). A new method for estimation and model selection: $\rho$-estimation. Inventiones mathematicae.](https://link.springer.com/article/10.1007/s00222-016-0673-5))
...
[12]  ([F.-X. Briol, A. Barp, A. B. Duncan and M. Girolami (2019). Statistical Inference for Gen erative Models via Maximum Mean Discrepancy. Preprint arXiv:1906.05944.](http://arxiv.org/abs/1906.05944))
